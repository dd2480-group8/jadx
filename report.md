...
| 34 | 125 | processSwitch@700-844@[RegionMaker.java](./jadx-core/src/main/java/jadx/core/dex/visitors/regions/RegionMaker.java) | Can easily be split into many small functions. It's heavily nested code and splitting it up would mean it would be easier to test different parts as well. It should be made public so it can be tested. Some if statements can actually be merged as well, because they test the same thing. An obvious case of this is the "outs.cardinality() > 1" checks. |
